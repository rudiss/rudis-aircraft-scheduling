...
const getFormattedSizes = (
  rules: RuleLengthArray = [],
  rule: string,
  isStyleObject = false
): any => {
...
export const mixins: IMixins["mixins"] = {
  ...
  setMargin: (margin, isStyleObject) =>
    getFormattedSizes(margin, "margin", isStyleObject),

  setPadding: (padding, isStyleObject) =>
    getFormattedSizes(padding, "padding", isStyleObject),
  ...
  setColor: (color) => {
    const selectedColor = color?.includes("#")
      ? `${color} !important`
      : `${colors[color || "black"]} !important`;
